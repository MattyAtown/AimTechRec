...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderJobs(jobs, container, isMatched = false) {
    container.innerHTML = "";
    if (jobs.length === 0) {
    ...
    jobs.forEach((job) => {
      ...
      box.innerHTML = `
        ...
        <p><strong>Salary:</strong> £${job.salary_min || job.salary || "Not listed"}</p>
        ${isMatched ? `<p><strong>Match:</strong> ${job.match}%</p>` : ""}
      `;
